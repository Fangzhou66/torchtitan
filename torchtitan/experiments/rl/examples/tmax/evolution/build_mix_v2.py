...
def tw_rows(
    ids_path: Path, disk_results: Path, tasks_parquet: Path, seed_pool: Path
    ...
    if not disk_results.exists():
        print(
            f"WARNING: {disk_results} missing -- every measured disk override "
            f"is being dropped, rows fall back to est_disk_mb",
            file=sys.stderr,
        )
...
def tmax_rows(
    tasks: Path, reaudit_parquet: Path, peaks_parquet: Path | None
) -> tuple[list[dict], list[str]]:
...
def main() -> None:
    ...
    ap.add_argument(
        "--out", required=True, type=Path, help="the seed file for new_root.py --mix"
    )
    ...
    tw_ids = (
        Path(args.tw_ids)
        if args.tw_ids
        else tw_src / "metadata" / "train_ready_ids.txt"
    )
    tasks_parquet = (
        Path(args.tasks_parquet)
        if args.tasks_parquet
        else tw_src / "metadata" / "tasks.parquet"
    )
    tmax_clean = root.data / "sources" / "tmax-clean" / "splits"
    tmax_tasks = (
        Path(args.tmax_tasks)
        if args.tmax_tasks
        else root.data / "sources" / "tmax-extract" / "tasks"
    )
    tmax_parquet = (
        Path(args.tmax_parquet) if args.tmax_parquet else tmax_clean / "reaudit.parquet"
    )
    tmax_peaks = (
        Path(args.tmax_peaks)
        if args.tmax_peaks
        else tmax_clean / "reaudit_full.parquet"
    )
    disk_results = (
        Path(args.disk_results)
        if args.disk_results
        else root.path / "results" / "disk_full.jsonl"
    )
    if not tmax_peaks.exists():
        print(
            f"WARNING: {tmax_peaks} missing -- every tmax row falls back to the "
            f"fleet default size",
            file=sys.stderr,
        )

    tw, tw_missing = tw_rows(tw_ids, disk_results, tasks_parquet, tw_src / "tasks")
    tm, tm_missing = tmax_rows(
        tmax_tasks, tmax_parquet, tmax_peaks if tmax_peaks.exists() else None
    )
    rows = tw + tm
    ...
    if tw_missing or tm_missing:
        print(
            f"WARNING: {len(tw_missing)} TW + {len(tm_missing)} tmax ids unresolved",
            file=sys.stderr,
        )
    if not args.apply:
